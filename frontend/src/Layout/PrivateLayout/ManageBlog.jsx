...
const BlogCard = styled(Link)`
  ...
  color: inherit;
  cursor: pointer;
  box-shadow: 5px 5px 5px rgba(0, 0, 0.1, 0.1);
`;
...
const debouncedSearch = debounce((value, setSearch) => {
  setSearch(value);
}, 400);
...
const ManageBlog = () => {
  ...
  const fetchBlogs = async () => {
    ...
    const response = await axios.get(
      `/blogs/findByBlogType/${selectedBlogTypes.join(",")}`,
      { params }
    );

    const newBlogImages = await Promise.all(
      response.result.data.map(async (blog) => {
      ...
      >
        {blogImages.map((blog, index) => (
          <BlogCard key={`${blog.id}-${index}`}>
            ...
            >
              <BlogImage
                src={blog.images || "/img/default-image.jpg"}
                alt={blog.title}
              />
              <div style={{ flex: 1 }}>
